...
from Attn import VOX_mean, activation_func, mySequential
...
class ResNetBasicBlock(ResNetResidualBlock):
    expansion = 1
    def __init__(self, in_channels, out_channels, num_classes, *args, **kwargs):
        super().__init__(in_channels, out_channels, *args, **kwargs)
        self.vox_op1 = VOX_mean(in_channels, out_channels, num_classes)
        self.vox_op2 = VOX_mean(out_channels * self.expansion, out_channels, num_classes)
        self.blocks = nn.ModuleList([
...
class ResNetLayer(nn.Module):
    def __init__(self, in_channels, out_channels, n_classes,
        ...
        self.res_blocks = nn.ModuleList([
            block(in_channels, out_channels, n_classes, *args, **kwargs),
            *[block(out_channels * block.expansion, 
                    out_channels, n_classes, *args, **kwargs) for _ in range(n - 1)]
        ])
...
class ResNetEncoder(nn.Module):
    def __init__(self, input_dim=3, output_dim=8, 
                ...
                depths=[2,2,2,2],
                attn_latents=[5, 10, 15, 25],
                 activation='relu', block=ResNetBasicBlock, *args, **kwargs):
        super().__init__()
        self.block_sizes = block_sizes
        self.attn_latents = attn_latents

        self.init_layer = mySequential(
            VOX_mean(input_dim, self.block_sizes[0], self.attn_latents[0],),
            activation_func(activation),
        )
                
        self.in_out_block_sizes = list(zip(block_sizes, block_sizes[1:]))
        self.layers = nn.ModuleList([ 
            ResNetLayer(block_sizes[0], block_sizes[0], attn_latents[0], 
                        n=depths[0], activation=activation, 
                        block=block,*args, **kwargs),
            *[ResNetLayer(in_channels * block.expansion, out_channels, n_classes,
                        n=n, activation=activation, 
                        block=block, *args, **kwargs) 
              for (in_channels, out_channels), n, n_classes, in zip(self.in_out_block_sizes, depths[1:], attn_latents[1:])],
            VOX_mean(self.in_out_block_sizes[-1][-1], output_dim, attn_latents[-1]),       
        ])
